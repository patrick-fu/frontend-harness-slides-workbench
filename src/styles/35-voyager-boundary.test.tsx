...
import VoyagerBoundary, {
  getMetadata,
  voyagerBoundaryTopic,
  VOYAGER_BOUNDARY_SOURCES,
  VOYAGER_BOUNDARY_TRANSITION_SCORE,
} from "./35-voyager-boundary";
import componentSource from "./35-voyager-boundary.tsx?raw";
...
describe("Voyager Boundary topic protocol", () => {
  it("exports the planned topic, navigation, sources, and exact transition score", () => {
    expect(voyagerBoundaryTopic.id).toBe("voyager-boundary");
    expect(voyagerBoundaryTopic.topic).toEqual({
      en: "Voyager Boundary",
      zh: "日球层边界",
    });
    expect(voyagerBoundaryTopic.navigation).toEqual({
      geometry: "path",
      carrier: "telemetry-trail",
      invocation: "keyboard-focus",
      feedback: "active-glow",
    });
    expect(voyagerBoundaryTopic.sources).toBe(VOYAGER_BOUNDARY_SOURCES);
    expect(voyagerBoundaryTopic.transitionScore).toBe(
      VOYAGER_BOUNDARY_TRANSITION_SCORE,
    );
  ...
  it("keeps five localized scenes and claims structurally aligned", () => {
    ...
    expect(english.id).toBe("retro-windows");
