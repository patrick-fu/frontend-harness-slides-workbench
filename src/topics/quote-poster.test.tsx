...
import quotePoster from "./quote-poster";
import type { TopicStageProps } from "../domain/topic";
import { runTopicContract } from "../testing/topic-contract";

runTopicContract(quotePoster);

const TopicStage = quotePoster.Stage;
...
describe("Quote Poster render coverage", () => {
...
describe("Quote Poster thumbnail mode", () => {
...
describe("Quote Poster navigation presence", () => {
  it("renders 5 nav tabs when not in thumbnail mode", () => {
...
describe("Quote Poster navigation behavior", () => {
  it("clicking nav calls onNavigate with (sceneId, 0)", () => {
    const { onNavigate } = renderStage({ scene: 1, beat: 0 });
    const tab2 = screen
      .getAllByRole("button")
      .find((el) => el.getAttribute("aria-label") === "Jump to scene 2");
...
describe("Quote Poster overflow check", () => {
...
describe("Quote Poster metadata structure", () => {
  it("keeps complete Topic metadata", () => {
    const meta = quotePoster.metadata.en;
    expect(quotePoster.styleId).toBe("spotlight-quote-poster");
