...
describe("Broadsheet — Scene/Beat coverage", () => {
  ...
  it("renders scene 1 beat 0 (front page masthead)", () => {
    ...
    expect(screen.getByTestId("broadsheet-root")).toBeInTheDocument();
  });
...
describe("Broadsheet — isThumbnail behavior", () => {
  ...
  it("onNavigate is undefined when isThumbnail=true", () => {
    ...
    expect(screen.queryByTestId("broadsheet-nav")).not.toBeInTheDocument();
...
describe("Broadsheet — Navigation interaction", () => {
...
describe("Broadsheet — Chinese language", () => {
  ...
  it("renders vertical writing mode for zh headline on scene 1", () => {
    renderStage({ scene: 1, beat: 0, language: "zh" });
    const headline = within(getActivePanel()).getByTestId("broadsheet-headline");
...
describe("Broadsheet — English language", () => {
...
describe("Broadsheet — Overflow check", () => {
...
// ─── 9. Static metadata retains the authored structure ──────────────────────

describe("Broadsheet — metadata structure", () => {
  let metaEn: typeof topic.metadata.en;
  let metaZh: typeof topic.metadata.zh;

  beforeEach(() => {
    metaEn = topic.metadata.en;
    metaZh = topic.metadata.zh;
  });

  it("returns all required top-level fields", () => {
    expect(topic.id).toBe("broadsheet");
    expect(topic.styleId).toBe("front-page-broadsheet");
    expect(topic.title).toEqual({ en: "Broadsheet", zh: "大报头版" });
