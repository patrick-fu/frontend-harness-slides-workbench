...
import styles from "./10-durable-tool.module.css";
...
function useFonts() {
  useEffect(() => {
    const id = "style-10-durable-tool-v2-fonts";
...
function getMetadata(lang: Lang): StyleMetadata {
  return {
    ...
    name: lang === "zh" ? "基准矩阵" : "Benchmark Matrix",
    theme: t(
      {
...
function CornerTabs({
  ...
  return (
    <nav className={styles.cornerTabs} aria-label="Scene navigation">
...
export default function DurableToolBenchmarkMatrixV2({
  scene,
  beat,
  language,
  isThumbnail,
...
export const durableToolTopic = defineStyleTopic({
  id: "durable-tool",
  topic: {
    en: "Durable Tool",
    zh: "耐用工具",
