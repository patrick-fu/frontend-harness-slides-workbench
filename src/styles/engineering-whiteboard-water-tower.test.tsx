...
import EngineeringWhiteboardWaterTower, {
  WATER_TOWER_SOURCES,
  WATER_TOWER_TRANSITION_SCORE,
  getMetadata,
  waterTowerTopic,
} from "./engineering-whiteboard-water-tower";
...
function renderTopic({
  ...
  reducedMotion = false,
  onNavigate = vi.fn(),
}: Partial<React.ComponentProps<typeof EngineeringWhiteboardWaterTower>> = {}) {
  return {
    onNavigate,
    ...render(
      <div data-testid="event-boundary" onClick={vi.fn()} style={{ width: 1920, height: 1080, containerType: "size" }}>
        <EngineeringWhiteboardWaterTower
          scene={scene}
          beat={beat}
          language={language}
...
describe("Engineering Whiteboard · Water Tower", () => {
  it("exports the planned semantic topic and exact transition score", () => {
    expect(waterTowerTopic.id).toBe("water-tower");
    expect(waterTowerTopic.topic).toEqual({
      en: "Water Tower",
      zh: "城市水塔",
    });
    expect(waterTowerTopic.navigation).toEqual({
    ...
    expect(waterTowerTopic.transitionScore).toBe(WATER_TOWER_TRANSITION_SCORE);
    expect(waterTowerTopic.sources).toBe(WATER_TOWER_SOURCES);
  });
  ...
  it("keeps five bilingual scenes structurally aligned with the 1-1-2-3-4 curve", () => {
    ...
    expect(en.id).toBe("engineering-whiteboard-explainer");
  ...
  it("routes every authored scene edge through its canonical transition", () => {
    ...
    const { rerender } = render(
      <div style={{ width: 1920, height: 1080, containerType: "size" }}>
        <EngineeringWhiteboardWaterTower scene={1} {...props} />
      </div>,
    );
    ...
    for (const [scene, transition] of [
      ...
      rerender(
        <div style={{ width: 1920, height: 1080, containerType: "size" }}>
          <EngineeringWhiteboardWaterTower scene={scene} {...props} />
        </div>,
      );
