...
import styles from "./28-field-route-signal.module.css";
...
function useFonts() {
  useEffect(() => {
    const id = "style-28-field-route-signal-v2-fonts";
...
export function getMetadata(lang: Lang): StyleMetadata {
  return {
    ...
    band: "craft-cultural",
    name: lang === "zh" ? "远征丝网版画" : "Expedition Screenprint",
...
function CoordinateRail({
  ...
  return (
    <nav className={styles.coordinateRail} aria-label={label}>
...
function ScreenGrain() {
  return (
    <svg className={styles.grainSvg} viewBox="0 0 1200 800" aria-hidden="true">
      <defs>
        <pattern
          id="field-route-signal-v2-grain"
          width="24"
          height="24"
      ...
      <rect width="1200" height="800" fill="url(#field-route-signal-v2-grain)" />
    </svg>
  );
}
...
export default function FieldRouteSignalV2({
  scene,
  beat,
  language,
  isThumbnail,
  ...
  return (
    <div className={styles.root} data-static={staticMode ? "true" : "false"}>
      <ScreenGrain />
...
export const fieldRouteSignalTopic = defineStyleTopic({
  id: "field-route",
  topic: {
    en: "Field Route",
    zh: "现场路线",
