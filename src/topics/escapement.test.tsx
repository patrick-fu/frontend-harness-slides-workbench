...
import topic from "./escapement";
import componentSource from "./escapement.tsx?raw";
import cssSource from "./escapement.module.css?inline";
...
describe("The Escapement Topic contract", () => {
  it("exports the planned topic, navigation profile, facts packet, and exact score", () => {
    expect(topic.id).toBe("escapement");
    expect(topic.styleId).toBe("operating-manual");
  ...
  it("ships at least three authoritative, claim-scoped HTTPS sources", () => {
    if (topic.evidence.kind !== "mixed") {
      throw new Error("Escapement must retain mixed evidence.");
    }
    expect(topic.evidence.sources.length).toBeGreaterThanOrEqual(3);
    for (const source of topic.evidence.sources) {
...
describe("The Escapement — render and mechanism", () => {
...
describe("The Escapement — navigation and deterministic modes", () => {
...
describe("The Escapement — motion and stage safety", () => {
