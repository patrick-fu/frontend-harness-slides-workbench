...
} from "./magazine-masthead-moth-experiment";
import styleClasses from "./magazine-masthead-moth-experiment.module.css";
import componentSource from "./magazine-masthead-moth-experiment.tsx?raw";
...
const cssSource = readFileSync(
  "src/styles/magazine-masthead-moth-experiment.module.css",
  "utf8",
);
...
describe("Magazine Masthead: moth experiment — topic contract", () => {
  it("exports the locked topic, specimen-card navigation, and authored score", () => {
    expect(mothExperimentTopic.id).toBe("moth-experiment");
    expect(mothExperimentTopic.topic).toEqual({
      en: "Moth Experiment",
      zh: "桦尺蛾实验",
    });
    ...
    expect(mothExperimentTopic.sources).toBe(MOTH_EXPERIMENT_SOURCES);
    expect(mothExperimentTopic.transitionScore).toBe(
      MOTH_EXPERIMENT_TRANSITION_SCORE,
    );
  ...
  it("keeps EN and ZH metadata aligned to the 2/4/1/3/1 curve", () => {
    ...
    expect(en.id).toBe("magazine-masthead");
