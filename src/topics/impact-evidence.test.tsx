...
describe("Impact Evidence — reading-first render", () => {
...
describe("Impact Evidence — source and transition contracts", () => {
  it("exports the exact low-motion transition score and applies all four edges", () => {
    ...
    expect(topic.transitionScore).toBe(impactEvidenceTransitionScore);
  ...
  it("carries authoritative HTTPS sources and a closed claim-to-source graph", () => {
    ...
    if (topic.evidence.kind !== "facts") {
      throw new Error("Impact Evidence must retain factual Evidence.");
    }
    expect(topic.evidence.sources).toBe(impactEvidenceSources);
...
describe("Impact Evidence — citation-chain navigation", () => {
