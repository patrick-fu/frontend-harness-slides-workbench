...
import OralToWritten, {
  getMetadata,
  ORAL_TO_WRITTEN_SOURCES,
  ORAL_TO_WRITTEN_TRANSITION_SCORE,
  oralToWrittenTopic,
} from "./warm-editorial-feature-oral-to-written";
import componentSource from "./warm-editorial-feature-oral-to-written.tsx?raw";
import styleClasses from "./warm-editorial-feature-oral-to-written.module.css";
...
const cssSource = readFileSync(
  "src/styles/warm-editorial-feature-oral-to-written.module.css",
  "utf8",
);
...
describe("Warm Editorial Feature: oral to written — topic contract", () => {
  it("exports the locked topic, chapter-index navigation, and transition score", () => {
    expect(oralToWrittenTopic.id).toBe("oral-to-written");
    expect(oralToWrittenTopic.topic).toEqual({
      en: "Oral to Written",
      zh: "史诗成文",
    });
    expect(oralToWrittenTopic.navigation).toEqual({
      geometry: "typographic-index",
      carrier: "oral-written-chapter-labels",
      invocation: "proximity-reveal",
      feedback: "active-glow",
    });
    expect(oralToWrittenTopic.sources).toBe(ORAL_TO_WRITTEN_SOURCES);
    expect(oralToWrittenTopic.transitionScore).toBe(
      ORAL_TO_WRITTEN_TRANSITION_SCORE,
    );
  ...
  it("keeps EN and ZH metadata aligned to the 2/3/3/3/1 reading curve", () => {
    ...
    expect(en.id).toBe("warm-editorial-feature");
...
describe("Warm Editorial Feature: oral to written — stage safety", () => {
  it("contains no loops, hotlinked visuals, legacy clones, or unsafe stage units", () => {
