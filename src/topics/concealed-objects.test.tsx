...
import styleClasses from "./concealed-objects.module.css";
import componentSource from "./concealed-objects.tsx?raw";

const { Stage } = concealedObjects;

runTopicContract(concealedObjects);
...
const cssSource = readFileSync(
  "src/topics/concealed-objects.module.css",
  "utf8",
);
...
describe("Analog Cutout Collage: concealed objects — topic contract", () => {
  it("exports the locked topic, wall-cache navigation, model, and score", () => {
    expect(concealedObjects.id).toBe("concealed-objects");
    expect(concealedObjects.title).toEqual({
      en: "Inside the Wall",
      zh: "墙中藏物",
    });
    expect(concealedObjects.modelId).toBe("GPT 5.6 Sol");
    expect(concealedObjects.navigation).toEqual({
      geometry: "card-miniature",
      carrier: "wall-cache-fragments",
      invocation: "click-expand",
      feedback: "active-glow",
    });
    expect(concealedObjects.evidence).toEqual(
      expect.objectContaining({ kind: "mixed", sources: CONCEALED_OBJECTS_SOURCES }),
    );
    expect(concealedObjects.transitionScore).toBe(
      CONCEALED_OBJECTS_TRANSITION_SCORE,
    );
  ...
  it("keeps EN and ZH metadata aligned to the 4/3/2/1/1 curve", () => {
    const en = concealedObjects.metadata.en;
    const zh = concealedObjects.metadata.zh;

    expect(concealedObjects.styleId).toBe("analog-cutout-collage");
...
describe("Analog Cutout Collage: concealed objects — stage-safe authored DOM", () => {
  it("uses original DOM/SVG/CSS without remote visual assets, loops, or transition clones", () => {
