...
1. 扩展 `SpatialSceneTrack` 的类型、CSS 与测试，使 21 primitives 真正可用；当前代码仍只有 8 种，本文的 21 种是计划，不是现成功能。
...
### Gate 3 — 49 个独立 sub-agent，17 个小批次

- 每个 Topic 由一个独立 sub-agent 负责实现、Review 与聚焦测试；共 49 个 sub-agent task。
- 同时最多 3 个生产 sub-agent；主 agent 负责共享文件、冲突处理、全局验证、提交、推送和部署。
- 每批 3 个 Topic（最后一批 1 个）。批内全部通过后立即 commit + push，再启动下一批。
- 每个 agent 只拿到：Style DNA、本文对应小节、facts packet、共享 contract、forbidden defaults；不得自行改题或复制旧 Topic 布局。

| Batch | Style # | 目的 |
|---:|---|---|
| 01 | 06, 07, 12 | Type / Emoji / engineering anchor |
| 02 | 18, 35, 37 | Editorial / digital / material anchor |
| 03 | 42, 01, 33 | Evidence anchor + static impact + mechanical diagram |
| 04 | 39, 49, 38 | Cinematic / object hero / manual |
| 05 | 40, 21, 05 | Chalk / vellum / cyanotype |
| 06 | 29, 27, 34 | Screenprint / specimen / glass |
| 07 | 32, 04, 09 | Agitprop / dialogue / route map |
| 08 | 22, 03, 36 | Solar poster / ceramic / organic narrative |
| 09 | 14, 11, 23 | Food material / pipeline / duotone type |
| 10 | 20, 16, 44 | Editorial / instrument / evidence brief |
| 11 | 13, 47, 17 | Cute SVG / ledger / diagnostic |
| 12 | 45, 46, 10 | Field notes / source diff / matrix |
| 13 | 31, 19, 28 | Brutalist / masthead / geometric force |
| 14 | 15, 08, 43 | Pairing / spotlight / decision record |
| 15 | 30, 41, 48 | Cassette / game / bento |
| 16 | 24, 02, 26 | Riso / Swiss / woodblock |
| 17 | 25 | Analog collage final singleton |

每批检查：聚焦 unit test、typecheck、registry/protocol、EN/ZH、frozen/thumbnail；必要时浏览器截图。每完成三批再运行一次 build 与更宽视觉抽查，避免局部测试长期掩盖集成问题。
...
本文已成为施工 source of truth。实施按 §9 的 17 个批次推进；所有批次、最终多-agent 验收、Vercel 生产部署与线上冒烟通过后才算完成。
