...
import MonarchMigration, {
  MONARCH_MIGRATION_SOURCES,
  MONARCH_MIGRATION_TRANSITION_SCORE,
  getMetadata,
  monarchMigrationTopic,
} from "./mid-century-grove-monarch-migration";
...
function renderStage(props: Partial<BespokeStyleProps> = {}) {
  ...
  const result = render(
    ...
    >
      <MonarchMigration {...componentProps} />
    </div>,
  );
  return {
    ...result,
...
describe("Mid-Century Grove / Monarch Migration — topic contract", () => {
  it("exports the planned topic, seasonal-halo navigation, and exact score", () => {
    expect(monarchMigrationTopic.id).toBe("monarch-migration");
    expect(monarchMigrationTopic.topic).toEqual({
      en: "Monarch Migration",
      zh: "帝王蝶迁徙",
    });
    expect(monarchMigrationTopic.navigation).toEqual({
      geometry: "ambient",
      carrier: "seasonal-halo",
      invocation: "keyboard-focus",
      feedback: "material-color-change",
    });
    expect(monarchMigrationTopic.sources).toBe(MONARCH_MIGRATION_SOURCES);
    expect(monarchMigrationTopic.transitionScore).toBe(
      MONARCH_MIGRATION_TRANSITION_SCORE,
    );
    expect(MONARCH_MIGRATION_TRANSITION_SCORE).toEqual({
      "1->2": "push-y",
      "2->3": "push-x",
      "3->4": "focus-swap",
      "4->5": "push-y",
  ...
  it("ships claim-scoped authoritative sources with explicit route boundaries", () => {
    expect(MONARCH_MIGRATION_SOURCES.length).toBeGreaterThanOrEqual(3);
    for (const source of MONARCH_MIGRATION_SOURCES) {
      expect(source.url).toMatch(/^https:\/\//);
      expect(source.authority.trim().length).toBeGreaterThan(0);
      expect(source.title.trim().length).toBeGreaterThan(8);
      expect(source.citation.trim().length).toBeGreaterThan(20);
      expect(source.supports.trim().length).toBeGreaterThan(50);
      expect(source.boundary.trim().length).toBeGreaterThan(50);
    }
  });
  ...
  it("keeps EN/ZH metadata aligned to the 1/3/2/4/1 narrative curve", () => {
    ...
    expect(en.id).toBe("mid-century-grove");
...
describe("Mid-Century Grove / Monarch Migration — render and evidence", () => {
...
describe("Mid-Century Grove / Monarch Migration — transitions and navigation", () => {
  it("renders all four authored transition edges", async () => {
    ...
    for (let scene = 2; scene <= 5; scene += 1) {
      view.rerender(
        <div style={{ width: 1920, height: 1080, containerType: "size" }}>
          <MonarchMigration {...baseProps} scene={scene} />
        </div>,
      );
      await waitFor(() => {
        expect(
...
describe("Mid-Century Grove / Monarch Migration — stage safety", () => {
