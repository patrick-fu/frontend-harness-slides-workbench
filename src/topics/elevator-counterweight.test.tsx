...
} from "./elevator-counterweight";
import { runTopicContract } from "../testing/topic-contract";
import componentSource from "./elevator-counterweight.tsx?raw";
import styleSource from "./elevator-counterweight.module.css?inline";

runTopicContract(elevatorCounterweight);

const TopicStage = elevatorCounterweight.Stage;
const getMetadata = (language: "en" | "zh") => elevatorCounterweight.metadata[language];
...
describe("Elevator Counterweight topic protocol", () => {
  it("declares the assigned topic, sources, navigation profile, and transition score", () => {
    expect(elevatorCounterweight.id).toBe("elevator-counterweight");
    expect(elevatorCounterweight.title).toEqual({
      en: "Counterweight",
      zh: "电梯配重",
    });
    expect(elevatorCounterweight.modelId).toBe("GPT 5.6 Sol");
    expect(elevatorCounterweight.navigation).toEqual({
      geometry: "path",
      carrier: "counterweight-cable",
      invocation: "proximity-reveal",
      feedback: "geometry-reflow",
    });
    expect(elevatorCounterweight.evidence).toEqual({
      kind: "facts",
      sources: ELEVATOR_COUNTERWEIGHT_SOURCES,
    });
    expect(elevatorCounterweight.transitionScore).toBe(
      ELEVATOR_COUNTERWEIGHT_TRANSITION_SCORE,
    );
  ...
  it("keeps five bilingual scenes structurally aligned with 4-1-2-2-1 beats", () => {
    ...
    expect(elevatorCounterweight.styleId).toBe("collaborative-pairing-board");
