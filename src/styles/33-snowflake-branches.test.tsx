...
import SnowflakeBranches, {
  getMetadata,
  SNOWFLAKE_BRANCHES_SOURCES,
  SNOWFLAKE_BRANCHES_TRANSITION_SCORE,
  snowflakeBranchesTopic,
} from "./33-snowflake-branches";
import componentSource from "./33-snowflake-branches.tsx?raw";
import styleSource from "./33-snowflake-branches.module.css?inline";
...
describe("Snowflake Branches topic protocol", () => {
  it("exports the planned facts packet, navigation, and exact transition score", () => {
    expect(snowflakeBranchesTopic.id).toBe("snowflake-branches");
    expect(snowflakeBranchesTopic.topic).toEqual({
      en: "Snowflake Branches",
      zh: "雪花分支",
    });
    expect(snowflakeBranchesTopic.navigation).toEqual({
      geometry: "object-controller",
      carrier: "crystal-morphology-dial",
      invocation: "keyboard-focus",
      feedback: "history-trail",
    });
    expect(snowflakeBranchesTopic.sources).toBe(SNOWFLAKE_BRANCHES_SOURCES);
    expect(snowflakeBranchesTopic.transitionScore).toBe(
      SNOWFLAKE_BRANCHES_TRANSITION_SCORE,
    );
  ...
  it("keeps five bilingual scene structures and claims aligned", () => {
    ...
    expect(english.id).toBe("mechanical-scoring-funnel");
    expect(english.band).toBe("craft-cultural");
