import type { TopicRegistry } from "./topic-catalog";

import productKeynoteDefinition from "../topics/product-keynote";
import quietLaunchDefinition from "../topics/quiet-launch";
import presolarGrainDefinition from "../topics/presolar-grain";
import lastFeatureCutDefinition from "../topics/last-feature-cut";
import solidStateBatteryDefinition from "../topics/solid-state-battery";

import swissGridDefinition from "../topics/swiss-grid";
import cleanMetricsDefinition from "../topics/clean-metrics";
import bridgeMovementDefinition from "../topics/bridge-movement";
import anatomyTimetableDefinition from "../topics/anatomy-timetable";
import airTrafficSeparationDefinition from "../topics/air-traffic-separation";

import ceramicCalmDefinition from "../topics/ceramic-calm";
import repairStrategyDefinition from "../topics/repair-strategy";
import stoneToSoilDefinition from "../topics/stone-to-soil";
import beautyUnfinishedDefinition from "../topics/beauty-unfinished";
import kintsugiEntropyDefinition from "../topics/kintsugi-entropy";

import dialogueStageDefinition from "../topics/dialogue-stage";
import betterQuestionDefinition from "../topics/better-question";
import vocalFoldsDefinition from "../topics/vocal-folds";
import rubberDuckDefinition from "../topics/rubber-duck";
import turingTestDilemmaDefinition from "../topics/turing-test-dilemma";

import blueprintDefinition from "../topics/blueprint";
import resiliencePlanDefinition from "../topics/resilience-plan";
import cometAnatomyDefinition from "../topics/comet-anatomy";
import drawingABridgeDefinition from "../topics/drawing-a-bridge";
import fresnelLensDraftingDefinition from "../topics/fresnel-lens-drafting";

import typePosterDefinition from "../topics/type-poster";
import oneConstraintDefinition from "../topics/one-constraint";
import beforeADefinition from "../topics/before-a";
import shipItDefinition from "../topics/ship-it";
import amdahlsLawDefinition from "../topics/amdahls-law";

import workshopBoardDefinition from "../topics/workshop-board";
import humanLoopDefinition from "../topics/human-loop";
import stadiumWaveDefinition from "../topics/stadium-wave";
import howWeNamedItDefinition from "../topics/how-we-named-it";
import rubberHoseAnimationDefinition from "../topics/rubber-hose-animation";

import quotePosterDefinition from "../topics/quote-poster";
import keptSentenceDefinition from "../topics/kept-sentence";
import freediveDefinition from "../topics/freedive";
import onQuittingWellDefinition from "../topics/on-quitting-well";
import shannonInformationDefinition from "../topics/shannon-information";

import subwayFlowDefinition from "../topics/subway-flow";
import releaseTracksDefinition from "../topics/release-tracks";
import teaChaRoutesDefinition from "../topics/tea-cha-routes";
import threeTeamsLaunchDefinition from "../topics/three-teams-launch";
import bgpAnycastRoutingDefinition from "../topics/bgp-anycast-routing";

import benchmarkDefinition from "../topics/benchmark";
import durableToolDefinition from "../topics/durable-tool";
import naturalClocksDefinition from "../topics/natural-clocks";
import buildBuyBorrowDefinition from "../topics/build-buy-borrow";
import solidStateDriveNandDefinition from "../topics/solid-state-drive-nand";

import pipelineDefinition from "../topics/pipeline";
import eventInsightDefinition from "../topics/event-insight";
import districtHeatDefinition from "../topics/district-heat";
import whereRequestGoesDefinition from "../topics/where-request-goes";
import zeroKnowledgeSnarkDefinition from "../topics/zero-knowledge-snark";

import fromPromptToPatchDefinition from "../topics/from-prompt-to-patch";
import waterTowerDefinition from "../topics/water-tower";
import tcpCongestionControlDefinition from "../topics/tcp-congestion-control";
import twoTrianglesOnePixelDefinition from "../topics/two-triangles-one-pixel";
import raftConsensusElectionDefinition from "../topics/raft-consensus-election";

import friendlyOnboardDefinition from "../topics/friendly-onboard";
import breathingOnboardDefinition from "../topics/breathing-onboard";
import chrysalisRebuildDefinition from "../topics/chrysalis-rebuild";
import firstWeekHereDefinition from "../topics/first-week-here";
import origamiCreasePatternsDefinition from "../topics/origami-crease-patterns";

import prepStationDefinition from "../topics/prep-station";
import cleanBriefDefinition from "../topics/clean-brief";
import cocoaFermentationDefinition from "../topics/cocoa-fermentation";
import rawLogsToReportDefinition from "../topics/raw-logs-to-report";
import sourdoughMicrobiomeDefinition from "../topics/sourdough-microbiome";

import pairingBoardDefinition from "../topics/pairing-board";
import sharedArtifactDefinition from "../topics/shared-artifact";
import elevatorCounterweightDefinition from "../topics/elevator-counterweight";
import humanReviewsAiDefinition from "../topics/human-reviews-ai";
import compilerFrontBackEndDefinition from "../topics/compiler-front-back-end";

import mixingConsoleDefinition from "../topics/mixing-console";
import operatingModelDefinition from "../topics/operating-model";
import tidalTimeDefinition from "../topics/tidal-time";
import tuningTheModelDefinition from "../topics/tuning-the-model";
import powerGridDispatchConsoleDefinition from "../topics/power-grid-dispatch-console";

import debugBoardDefinition from "../topics/debug-board";
import learningIncidentDefinition from "../topics/learning-incident";
import acousticCrackDefinition from "../topics/acoustic-crack";
import safeToDeployDefinition from "../topics/safe-to-deploy";
import linuxOomKillerDefinition from "../topics/linux-oom-killer";

import broadsheetDefinition from "../topics/broadsheet";
import afterLaunchDefinition from "../topics/after-launch";
import rogueWaveDefinition from "../topics/rogue-wave";
import dayFeedStoppedDefinition from "../topics/day-feed-stopped";
import deepSeaCablesCutDefinition from "../topics/deep-sea-cables-cut";

import mastheadDefinition from "../topics/masthead";
import productCoverDefinition from "../topics/product-cover";
import mothExperimentDefinition from "../topics/moth-experiment";
import comebackIssueDefinition from "../topics/comeback-issue";
import mechanicalWatchEscapementDefinition from "../topics/mechanical-watch-escapement";

import editorialFeatureDefinition from "../topics/editorial-feature";
import usefulWeekDefinition from "../topics/useful-week";
import oralToWrittenDefinition from "../topics/oral-to-written";
import letterToPastSelfDefinition from "../topics/letter-to-past-self";
import lostLibraryAlexandriaDefinition from "../topics/lost-library-alexandria";

import scholarNotesDefinition from "../topics/scholar-notes";
import marginArgumentDefinition from "../topics/margin-argument";
import hiddenTextDefinition from "../topics/hidden-text";
import whatAncientsKnewDefinition from "../topics/what-ancients-knew";
import principiaMathematicaProofDefinition from "../topics/principia-mathematica-proof";

import biennalePosterDefinition from "../topics/biennale-poster";
import publicLightDefinition from "../topics/public-light";
import ironFromStarsDefinition from "../topics/iron-from-stars";
import festivalSlowIdeasDefinition from "../topics/festival-slow-ideas";
import trombeWallPassiveSolarDefinition from "../topics/trombe-wall-passive-solar";

import sessionPosterDefinition from "../topics/session-poster";
import fiveTakesDefinition from "../topics/five-takes";
import danceNotationDefinition from "../topics/dance-notation";
import cutInOneTakeDefinition from "../topics/cut-in-one-take";
import magneticTapeSplicingDefinition from "../topics/magnetic-tape-splicing";

import risoZineDefinition from "../topics/riso-zine";
import communityPrintDefinition from "../topics/community-print";
import sevenBluesDefinition from "../topics/seven-blues";
import makeSomethingWeeklyDefinition from "../topics/make-something-weekly";
import pirateRadioBroadcastDefinition from "../topics/pirate-radio-broadcast";

import cutoutCollageDefinition from "../topics/cutout-collage";
import rebuiltArchiveDefinition from "../topics/rebuilt-archive";
import concealedObjectsDefinition from "../topics/concealed-objects";
import piecingIdeaTogetherDefinition from "../topics/piecing-idea-together";
import continentalDriftPangaeaDefinition from "../topics/continental-drift-pangaea";

import woodblockDefinition from "../topics/woodblock";
import tideMapDefinition from "../topics/tide-map";
import whistledLanguageDefinition from "../topics/whistled-language";
import aRiversJourneyDefinition from "../topics/a-rivers-journey";
import kuroshioCurrentVoyageDefinition from "../topics/kuroshio-current-voyage";

import specimenPlateDefinition from "../topics/specimen-plate";
import growthSignalsDefinition from "../topics/growth-signals";
import leafStomataDefinition from "../topics/leaf-stomata";
import anatomyOfAnIdeaDefinition from "../topics/anatomy-of-an-idea";
import ginkgoBilobaLivingFossilDefinition from "../topics/ginkgo-biloba-living-fossil";

import decoGalaDefinition from "../topics/deco-gala";
import infrastructureGalaDefinition from "../topics/infrastructure-gala";
import reinforcedConcreteDefinition from "../topics/reinforced-concrete";
import grandUnveilingDefinition from "../topics/grand-unveiling";
import dieselStreamlinerTrainDefinition from "../topics/diesel-streamliner-train";

import expeditionPrintDefinition from "../topics/expedition-print";
import fieldRouteDefinition from "../topics/field-route";
import saharanDustDefinition from "../topics/saharan-dust";
import mappingUnknownGroundDefinition from "../topics/mapping-unknown-ground";
import marianaTrenchDescentDefinition from "../topics/mariana-trench-descent";

import cassettePackDefinition from "../topics/cassette-pack";
import releaseMixtapeDefinition from "../topics/release-mixtape";
import iceCoreArchiveDefinition from "../topics/ice-core-archive";
import greatestHitsVol1Definition from "../topics/greatest-hits-vol1";
import floppyDiskInterleavingDefinition from "../topics/floppy-disk-interleaving";

import brutalistBulletinDefinition from "../topics/brutalist-bulletin";
import hardThingDefinition from "../topics/hard-thing";
import sinkingDeltaDefinition from "../topics/sinking-delta";
import readBeforeMergeDefinition from "../topics/read-before-merge";
import zeroDayExploitMitigationDefinition from "../topics/zero-day-exploit-mitigation";

import redWedgeDefinition from "../topics/red-wedge";
import orgMoveDefinition from "../topics/org-move";
import pneumaticPostDefinition from "../topics/pneumatic-post";
import refactorTheSystemDefinition from "../topics/refactor-the-system";
import linearTypesMemorySafetyDefinition from "../topics/linear-types-memory-safety";

import scoringFunnelDefinition from "../topics/scoring-funnel";
import priorityScoreDefinition from "../topics/priority-score";
import snowflakeBranchesDefinition from "../topics/snowflake-branches";
import triageTheBacklogDefinition from "../topics/triage-the-backlog";
import ventureCapitalDealFunnelDefinition from "../topics/venture-capital-deal-funnel";

import liquidGlassDefinition from "../topics/liquid-glass";
import spatialBriefDefinition from "../topics/spatial-brief";
import safetyGlassDefinition from "../topics/safety-glass";
import layersOfAProductDefinition from "../topics/layers-of-a-product";
import totalInternalReflectionWaveguideDefinition from "../topics/total-internal-reflection-waveguide";

import retroDesktopDefinition from "../topics/retro-desktop";
import toolchainDeskDefinition from "../topics/toolchain-desk";
import voyagerBoundaryDefinition from "../topics/voyager-boundary";
import setupExeDefinition from "../topics/setup-exe";
import win95PlugAndPlayDefinition from "../topics/win95-plug-and-play";

import botanicalBrandDefinition from "../topics/botanical-brand";
import calmGrowthDefinition from "../topics/calm-growth";
import monarchMigrationDefinition from "../topics/monarch-migration";
import growingSlowlyOnPurposeDefinition from "../topics/growing-slowly-on-purpose";
import eamesLoungeChairMoldingDefinition from "../topics/eames-lounge-chair-molding";

import afterHoursDefinition from "../topics/after-hours";
import betaSalonDefinition from "../topics/beta-salon";
import urushiCureDefinition from "../topics/urushi-cure";
import theMidnightReleaseDefinition from "../topics/the-midnight-release";
import bespokePerfumePyramidDefinition from "../topics/bespoke-perfume-pyramid";

import manualDefinition from "../topics/manual";
import habitRunbookDefinition from "../topics/habit-runbook";
import escapementDefinition from "../topics/escapement";
import rotateTheSecretsDefinition from "../topics/rotate-the-secrets";
import issDockingProcedureDefinition from "../topics/iss-docking-procedure";

import titleCardDefinition from "../topics/title-card";
import systemActsDefinition from "../topics/system-acts";
import whaleFallDefinition from "../topics/whale-fall";
import chapterZeroDefinition from "../topics/chapter-zero";
import blackHoleEventHorizonDefinition from "../topics/black-hole-event-horizon";

import chalkTalkDefinition from "../topics/chalk-talk";
import shortcutDefinition from "../topics/shortcut";
import hearingPathDefinition from "../topics/hearing-path";
import derivingBigODefinition from "../topics/deriving-big-o";
import eulerIdentityDerivationDefinition from "../topics/euler-identity-derivation";

import bossFightDefinition from "../topics/boss-fight";
import latencyBossDefinition from "../topics/latency-boss";
import eggMimicryDefinition from "../topics/egg-mimicry";
import defeatingTechDebtDefinition from "../topics/defeating-tech-debt";
import databaseDeadlockDuelDefinition from "../topics/database-deadlock-duel";

import researchMemoDefinition from "../topics/research-memo";
import smallTeamDefinition from "../topics/small-team";
import impactEvidenceDefinition from "../topics/impact-evidence";
import whyUsersChurnDefinition from "../topics/why-users-churn";
import agenticMemoryHierarchyDefinition from "../topics/agentic-memory-hierarchy";

import decisionRecordDefinition from "../topics/decision-record";
import boundaryDefinition from "../topics/boundary";
import standardTimeDefinition from "../topics/standard-time";
import whyWeChoseMonorepoDefinition from "../topics/why-we-chose-monorepo";
import whyWeChoseLsmTreeDefinition from "../topics/why-we-chose-lsm-tree";

import issueBriefDefinition from "../topics/issue-brief";
import agentPickupDefinition from "../topics/agent-pickup";
import ozoneHoleDefinition from "../topics/ozone-hole";
import flakyTestRootCauseDefinition from "../topics/flaky-test-root-cause";
import memoryLeakInV8ClosureDefinition from "../topics/memory-leak-in-v8-closure";

import fieldNotesDefinition from "../topics/field-notes";
import platformStudyDefinition from "../topics/platform-study";
import ancientSoundDefinition from "../topics/ancient-sound";
import shadowingSupportDefinition from "../topics/shadowing-support";
import tokyoMetroPassengerFlowDefinition from "../topics/tokyo-metro-passenger-flow";

import sourceDiffDefinition from "../topics/source-diff";
import flowRewriteDefinition from "../topics/flow-rewrite";
import readingRosettaDefinition from "../topics/reading-rosetta";
import killingAGodObjectDefinition from "../topics/killing-a-god-object";
import loopTilingCacheDiffDefinition from "../topics/loop-tiling-cache-diff";

import checklistLedgerDefinition from "../topics/checklist-ledger";
import launchLedgerDefinition from "../topics/launch-ledger";
import pigmentWithoutTouchDefinition from "../topics/pigment-without-touch";
import closeTheQuarterDefinition from "../topics/close-the-quarter";
import falcon9PreflightChecklistDefinition from "../topics/falcon9-preflight-checklist";

import contextBentoDefinition from "../topics/context-bento";
import handoffBoxDefinition from "../topics/handoff-box";
import lichenPartnersDefinition from "../topics/lichen-partners";
import everythingTheInternNeedsDefinition from "../topics/everything-the-intern-needs";
import wasmSimdCompilerBoxDefinition from "../topics/wasm-simd-compiler-box";

import objectMetaphorDefinition from "../topics/object-metaphor";
import recoveryKitDefinition from "../topics/recovery-kit";
import cocoonToClothDefinition from "../topics/cocoon-to-cloth";
import onboardingToolkitDefinition from "../topics/onboarding-toolkit";
import gyroscopeAttitudeControlDefinition from "../topics/gyroscope-attitude-control";

/**
 * The sole hand-maintained ordering source for every Style Group and Topic.
 */
export const TOPIC_REGISTRY = [
  [
    productKeynoteDefinition,
    quietLaunchDefinition,
    presolarGrainDefinition,
    lastFeatureCutDefinition,
    solidStateBatteryDefinition,
  ],
  [
    swissGridDefinition,
    cleanMetricsDefinition,
    bridgeMovementDefinition,
    anatomyTimetableDefinition,
    airTrafficSeparationDefinition,
  ],
  [
    ceramicCalmDefinition,
    repairStrategyDefinition,
    stoneToSoilDefinition,
    beautyUnfinishedDefinition,
    kintsugiEntropyDefinition,
  ],
  [
    dialogueStageDefinition,
    betterQuestionDefinition,
    vocalFoldsDefinition,
    rubberDuckDefinition,
    turingTestDilemmaDefinition,
  ],
  [
    blueprintDefinition,
    resiliencePlanDefinition,
    cometAnatomyDefinition,
    drawingABridgeDefinition,
    fresnelLensDraftingDefinition,
  ],
  [
    typePosterDefinition,
    oneConstraintDefinition,
    beforeADefinition,
    shipItDefinition,
    amdahlsLawDefinition,
  ],
  [
    workshopBoardDefinition,
    humanLoopDefinition,
    stadiumWaveDefinition,
    howWeNamedItDefinition,
    rubberHoseAnimationDefinition,
  ],
  [
    quotePosterDefinition,
    keptSentenceDefinition,
    freediveDefinition,
    onQuittingWellDefinition,
    shannonInformationDefinition,
  ],
  [
    subwayFlowDefinition,
    releaseTracksDefinition,
    teaChaRoutesDefinition,
    threeTeamsLaunchDefinition,
    bgpAnycastRoutingDefinition,
  ],
  [
    benchmarkDefinition,
    durableToolDefinition,
    naturalClocksDefinition,
    buildBuyBorrowDefinition,
    solidStateDriveNandDefinition,
  ],
  [
    pipelineDefinition,
    eventInsightDefinition,
    districtHeatDefinition,
    whereRequestGoesDefinition,
    zeroKnowledgeSnarkDefinition,
  ],
  [
    fromPromptToPatchDefinition,
    waterTowerDefinition,
    tcpCongestionControlDefinition,
    twoTrianglesOnePixelDefinition,
    raftConsensusElectionDefinition,
  ],
  [
    friendlyOnboardDefinition,
    breathingOnboardDefinition,
    chrysalisRebuildDefinition,
    firstWeekHereDefinition,
    origamiCreasePatternsDefinition,
  ],
  [
    prepStationDefinition,
    cleanBriefDefinition,
    cocoaFermentationDefinition,
    rawLogsToReportDefinition,
    sourdoughMicrobiomeDefinition,
  ],
  [
    pairingBoardDefinition,
    sharedArtifactDefinition,
    elevatorCounterweightDefinition,
    humanReviewsAiDefinition,
    compilerFrontBackEndDefinition,
  ],
  [
    mixingConsoleDefinition,
    operatingModelDefinition,
    tidalTimeDefinition,
    tuningTheModelDefinition,
    powerGridDispatchConsoleDefinition,
  ],
  [
    debugBoardDefinition,
    learningIncidentDefinition,
    acousticCrackDefinition,
    safeToDeployDefinition,
    linuxOomKillerDefinition,
  ],
  [
    broadsheetDefinition,
    afterLaunchDefinition,
    rogueWaveDefinition,
    dayFeedStoppedDefinition,
    deepSeaCablesCutDefinition,
  ],
  [
    mastheadDefinition,
    productCoverDefinition,
    mothExperimentDefinition,
    comebackIssueDefinition,
    mechanicalWatchEscapementDefinition,
  ],
  [
    editorialFeatureDefinition,
    usefulWeekDefinition,
    oralToWrittenDefinition,
    letterToPastSelfDefinition,
    lostLibraryAlexandriaDefinition,
  ],
  [
    scholarNotesDefinition,
    marginArgumentDefinition,
    hiddenTextDefinition,
    whatAncientsKnewDefinition,
    principiaMathematicaProofDefinition,
  ],
  [
    biennalePosterDefinition,
    publicLightDefinition,
    ironFromStarsDefinition,
    festivalSlowIdeasDefinition,
    trombeWallPassiveSolarDefinition,
  ],
  [
    sessionPosterDefinition,
    fiveTakesDefinition,
    danceNotationDefinition,
    cutInOneTakeDefinition,
    magneticTapeSplicingDefinition,
  ],
  [
    risoZineDefinition,
    communityPrintDefinition,
    sevenBluesDefinition,
    makeSomethingWeeklyDefinition,
    pirateRadioBroadcastDefinition,
  ],
  [
    cutoutCollageDefinition,
    rebuiltArchiveDefinition,
    concealedObjectsDefinition,
    piecingIdeaTogetherDefinition,
    continentalDriftPangaeaDefinition,
  ],
  [
    woodblockDefinition,
    tideMapDefinition,
    whistledLanguageDefinition,
    aRiversJourneyDefinition,
    kuroshioCurrentVoyageDefinition,
  ],
  [
    specimenPlateDefinition,
    growthSignalsDefinition,
    leafStomataDefinition,
    anatomyOfAnIdeaDefinition,
    ginkgoBilobaLivingFossilDefinition,
  ],
  [
    decoGalaDefinition,
    infrastructureGalaDefinition,
    reinforcedConcreteDefinition,
    grandUnveilingDefinition,
    dieselStreamlinerTrainDefinition,
  ],
  [
    expeditionPrintDefinition,
    fieldRouteDefinition,
    saharanDustDefinition,
    mappingUnknownGroundDefinition,
    marianaTrenchDescentDefinition,
  ],
  [
    cassettePackDefinition,
    releaseMixtapeDefinition,
    iceCoreArchiveDefinition,
    greatestHitsVol1Definition,
    floppyDiskInterleavingDefinition,
  ],
  [
    brutalistBulletinDefinition,
    hardThingDefinition,
    sinkingDeltaDefinition,
    readBeforeMergeDefinition,
    zeroDayExploitMitigationDefinition,
  ],
  [
    redWedgeDefinition,
    orgMoveDefinition,
    pneumaticPostDefinition,
    refactorTheSystemDefinition,
    linearTypesMemorySafetyDefinition,
  ],
  [
    scoringFunnelDefinition,
    priorityScoreDefinition,
    snowflakeBranchesDefinition,
    triageTheBacklogDefinition,
    ventureCapitalDealFunnelDefinition,
  ],
  [
    liquidGlassDefinition,
    spatialBriefDefinition,
    safetyGlassDefinition,
    layersOfAProductDefinition,
    totalInternalReflectionWaveguideDefinition,
  ],
  [
    retroDesktopDefinition,
    toolchainDeskDefinition,
    voyagerBoundaryDefinition,
    setupExeDefinition,
    win95PlugAndPlayDefinition,
  ],
  [
    botanicalBrandDefinition,
    calmGrowthDefinition,
    monarchMigrationDefinition,
    growingSlowlyOnPurposeDefinition,
    eamesLoungeChairMoldingDefinition,
  ],
  [
    afterHoursDefinition,
    betaSalonDefinition,
    urushiCureDefinition,
    theMidnightReleaseDefinition,
    bespokePerfumePyramidDefinition,
  ],
  [
    manualDefinition,
    habitRunbookDefinition,
    escapementDefinition,
    rotateTheSecretsDefinition,
    issDockingProcedureDefinition,
  ],
  [
    titleCardDefinition,
    systemActsDefinition,
    whaleFallDefinition,
    chapterZeroDefinition,
    blackHoleEventHorizonDefinition,
  ],
  [
    chalkTalkDefinition,
    shortcutDefinition,
    hearingPathDefinition,
    derivingBigODefinition,
    eulerIdentityDerivationDefinition,
  ],
  [
    bossFightDefinition,
    latencyBossDefinition,
    eggMimicryDefinition,
    defeatingTechDebtDefinition,
    databaseDeadlockDuelDefinition,
  ],
  [
    researchMemoDefinition,
    smallTeamDefinition,
    impactEvidenceDefinition,
    whyUsersChurnDefinition,
    agenticMemoryHierarchyDefinition,
  ],
  [
    decisionRecordDefinition,
    boundaryDefinition,
    standardTimeDefinition,
    whyWeChoseMonorepoDefinition,
    whyWeChoseLsmTreeDefinition,
  ],
  [
    issueBriefDefinition,
    agentPickupDefinition,
    ozoneHoleDefinition,
    flakyTestRootCauseDefinition,
    memoryLeakInV8ClosureDefinition,
  ],
  [
    fieldNotesDefinition,
    platformStudyDefinition,
    ancientSoundDefinition,
    shadowingSupportDefinition,
    tokyoMetroPassengerFlowDefinition,
  ],
  [
    sourceDiffDefinition,
    flowRewriteDefinition,
    readingRosettaDefinition,
    killingAGodObjectDefinition,
    loopTilingCacheDiffDefinition,
  ],
  [
    checklistLedgerDefinition,
    launchLedgerDefinition,
    pigmentWithoutTouchDefinition,
    closeTheQuarterDefinition,
    falcon9PreflightChecklistDefinition,
  ],
  [
    contextBentoDefinition,
    handoffBoxDefinition,
    lichenPartnersDefinition,
    everythingTheInternNeedsDefinition,
    wasmSimdCompilerBoxDefinition,
  ],
  [
    objectMetaphorDefinition,
    recoveryKitDefinition,
    cocoonToClothDefinition,
    onboardingToolkitDefinition,
    gyroscopeAttitudeControlDefinition,
  ],
] as const satisfies TopicRegistry;
