...
import SafetyGlass, {
  ...
  getMetadata,
  safetyGlassTopic,
} from "./liquid-glass-safety-glass";
import componentSource from "./liquid-glass-safety-glass.tsx?raw";
import cssSource from "./liquid-glass-safety-glass.module.css?inline";
...
describe("Style 34 / Safety Glass — coordinated topic contract", () => {
  it("exports the planned topic, navigation profile, facts packet, and exact score", () => {
    expect(safetyGlassTopic.id).toBe("safety-glass");
    expect(safetyGlassTopic.topic).toEqual({
      en: "Safety Glass",
      zh: "夹层玻璃",
    });
    expect(safetyGlassTopic.navigation).toEqual({
      geometry: "card-miniature",
      carrier: "glass-lamina-stack",
      invocation: "drag-scrub",
      feedback: "history-trail",
    });
    expect(safetyGlassTopic.sources).toBe(SAFETY_GLASS_SOURCES);
    expect(safetyGlassTopic.transitionScore).toBe(
      SAFETY_GLASS_TRANSITION_SCORE,
    );
  ...
  it("keeps five bilingual scenes aligned to the 1-2-4-2-1 beat curve", () => {
    ...
    expect(english.id).toBe("liquid-glass");
...
describe("Style 34 / Safety Glass — material narrative", () => {
...
describe("Style 34 / Safety Glass — lamina-stack navigation", () => {
...
describe("Style 34 / Safety Glass — motion and stage safety", () => {
