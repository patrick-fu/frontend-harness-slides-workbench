...
import StandardTimeDecisionRecord, {
  getMetadata,
  standardTimeClaims,
...
} from "./decision-record-standard-time";
import componentSource from "./decision-record-standard-time.tsx?raw";
...
function StageFrame({
  ...
  return (
    ...
    >
      <StandardTimeDecisionRecord {...props} />
    </div>
  );
}
...
function renderStage(
  ...
  let currentProps: BespokeStyleProps = {
    ...BASE_PROPS,
    onNavigate: vi.fn(),
    ...overrides,
  };
...
describe("Decision Record / Standard Time — protocol and research packet", () => {
  it("exports the assigned topic, model, navigation profile, and transition score", () => {
    expect(standardTimeTopic.id).toBe("standard-time");
    expect(standardTimeTopic.topic).toEqual({
      en: "Standard Time",
      zh: "标准时",
    });
    ...
    expect(standardTimeTopic.sources).toBe(standardTimeSources);
    expect(standardTimeTopic.transitionScore).toBe(standardTimeTransitionScore);
  ...
  it("keeps English and Chinese metadata aligned to the 4-1-2-2-1 curve", () => {
    ...
    expect(en.id).toBe("decision-record");
