...
} from "./checklist-ledger-pigment-without-touch";
import componentSource from "./checklist-ledger-pigment-without-touch.tsx?raw";

const cssSource = readFileSync(
  "src/styles/checklist-ledger-pigment-without-touch.module.css",
  "utf8",
) as string;
...
describe("Style 47 / Pigment Without Touch — coordinated topic contract", () => {
  it("exports the planned topic, swatch-rail navigation, facts packet, and score", () => {
    expect(pigmentWithoutTouchTopic.id).toBe("pigment-without-touch");
    expect(pigmentWithoutTouchTopic.topic).toEqual({
      en: "Pigment Without Touch",
      zh: "无损识色",
    });
    expect(pigmentWithoutTouchTopic.navigation).toEqual({
      geometry: "edge-scale",
      carrier: "pigment-swatch-rail",
      invocation: "gesture-hold",
      feedback: "active-glow",
    });
    expect(pigmentWithoutTouchTopic.sources).toBe(
      PIGMENT_WITHOUT_TOUCH_SOURCES,
    );
    expect(pigmentWithoutTouchTopic.transitionScore).toBe(
      PIGMENT_WITHOUT_TOUCH_TRANSITION_SCORE,
    );
  ...
  it("keeps five bilingual scenes aligned to the 2-4-1-3-1 curve", () => {
    ...
    expect(english.id).toBe("checklist-ledger");
    expect(english.band).toBe("text-report");
...
describe("Style 47 / Pigment Without Touch — evidence states", () => {
...
describe("Style 47 / Pigment Without Touch — pigment swatch rail", () => {
...
describe("Style 47 / Pigment Without Touch — stage and asset safety", () => {
