...
import HearingPath, {
  ...
  getMetadata,
  hearingPathTopic,
} from "./40-hearing-path";
import componentSource from "./40-hearing-path.tsx?raw";
import cssSource from "./40-hearing-path.module.css?inline";
...
describe("Style 40 / How Hearing Begins — coordinated topic contract", () => {
  it("exports the planned topic, navigation profile, facts packet, and score", () => {
    expect(hearingPathTopic.id).toBe("hearing-path");
    expect(hearingPathTopic.topic).toEqual({
      en: "How Hearing Begins",
      zh: "听觉起点",
    });
    expect(hearingPathTopic.navigation).toEqual({
      geometry: "path",
      carrier: "auditory-pathway",
      invocation: "gesture-hold",
      feedback: "history-trail",
    });
    expect(hearingPathTopic.sources).toBe(HEARING_PATH_SOURCES);
    expect(hearingPathTopic.transitionScore).toBe(
      HEARING_PATH_TRANSITION_SCORE,
    );
  ...
  it("keeps five bilingual scenes aligned to the 1-1-2-3-4 curve", () => {
    ...
    expect(english.id).toBe("blackboard-chalk-talk");
...
describe("Style 40 / How Hearing Begins — derivation and factual states", () => {
...
describe("Style 40 / How Hearing Begins — auditory-pathway navigation", () => {
...
describe("Style 40 / How Hearing Begins — motion and stage safety", () => {
