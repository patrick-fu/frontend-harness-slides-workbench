...
describe("Style 27 / Leaf Stomata — coordinated topic contract", () => {
  it("exports the planned topic, navigation profile, and exact transition score", () => {
    expect(leafStomataTopic.id).toBe("leaf-stomata");
    expect(leafStomataTopic.topic).toEqual({
      en: "Leaf Stomata",
      zh: "叶片气孔",
    });
    expect(leafStomataTopic.navigation).toEqual({
      geometry: "ambient",
      carrier: "specimen-registration-marks",
      invocation: "drag-scrub",
      feedback: "geometry-reflow",
    });
    expect(leafStomataTopic.sources).toBe(LEAF_STOMATA_SOURCES);
    expect(leafStomataTopic.transitionScore).toBe(
      LEAF_STOMATA_TRANSITION_SCORE,
    );
  ...
  it("keeps EN/ZH metadata aligned to the 1/2/4/2/1 beat curve", () => {
    ...
    expect(en.id).toBe("botanical-specimen-plate");
...
describe("Style 27 / Leaf Stomata — render and scientific narrative", () => {
...
describe("Style 27 / Leaf Stomata — transitions and specimen registration", () => {
  it("renders all four planned transition edges", () => {
    ...
    for (let scene = 2; scene <= 5; scene += 1) {
      view.rerender(
        <div style={{ width: 1920, height: 1080, containerType: "size" }}>
          <LeafStomata {...baseProps} scene={scene} />
        </div>,
      );
      expect(
...
describe("Style 27 / Leaf Stomata — fixed-stage safety", () => {
