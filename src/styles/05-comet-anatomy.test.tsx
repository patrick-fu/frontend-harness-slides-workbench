...
import CometAnatomy, {
  COMET_ANATOMY_SOURCES,
  COMET_ANATOMY_TRANSITION_SCORE,
  cometAnatomyTopic,
  getMetadata,
} from "./05-comet-anatomy";
...
describe("Style 05 / Comet Anatomy — topic contract", () => {
  it("exports the planned topic, navigation profile, and exact transition score", () => {
    expect(cometAnatomyTopic.id).toBe("comet-anatomy");
    expect(cometAnatomyTopic.topic).toEqual({
      en: "Comet Anatomy",
      zh: "彗星解剖",
    });
    expect(cometAnatomyTopic.navigation).toEqual({
      geometry: "edge-scale",
      carrier: "comet-sectional-scale",
      invocation: "auto-hide",
      feedback: "next-state-preview",
    });
    expect(cometAnatomyTopic.sources).toBe(COMET_ANATOMY_SOURCES);
    expect(cometAnatomyTopic.transitionScore).toBe(
      COMET_ANATOMY_TRANSITION_SCORE,
    );
    expect(COMET_ANATOMY_TRANSITION_SCORE).toEqual({
      "1->2": "linear-wipe",
      "2->3": "push-x",
      "3->4": "iris-open",
      "4->5": "dip-to-color",
  ...
  it("ships claim-scoped authoritative sources and explicit evidence boundaries", () => {
    expect(COMET_ANATOMY_SOURCES.length).toBeGreaterThanOrEqual(3);
    for (const source of COMET_ANATOMY_SOURCES) {
  ...
  it("keeps EN/ZH metadata aligned to the 1/2/4/2/1 narrative curve", () => {
    ...
    expect(en.id).toBe("cyanotype-drafting-table");
...
describe("Style 05 / Comet Anatomy — render and evidence", () => {
...
describe("Style 05 / Comet Anatomy — transitions and sectional scale", () => {
  it("renders all four authored transition edges", async () => {
    ...
    for (let scene = 2; scene <= 5; scene += 1) {
      view.rerender(
        <div style={{ width: 1920, height: 1080, containerType: "size" }}>
          <CometAnatomy {...baseProps} scene={scene} />
        </div>,
      );
      await waitFor(() => {
        expect(
...
describe("Style 05 / Comet Anatomy — fixed-stage safety", () => {
