...
} from "./whistled-language";
import componentSource from "./whistled-language.tsx?raw";
...
describe("Whistled Language — topic packet", () => {
  it("exports the planned Topic, mountain-peak navigation, model, and transition score", () => {
    expect(topic.id).toBe("whistled-language");
    expect(topic.title).toEqual({
      en: "Whistled Language",
      zh: "口哨语言",
    });
    expect(topic.styleId).toBe("woodblock-floating-world");
  ...
  it("keeps EN/ZH metadata aligned to the 4-3-2-1-1 curve", () => {
    for (const language of ["en", "zh"] as const) {
      const metadata = metadataFor(language);
...
describe("Whistled Language — scenes and boundaries", () => {
...
describe("Whistled Language — transitions and mountain peaks", () => {
...
describe("Whistled Language — deterministic stage-safe DOM", () => {
