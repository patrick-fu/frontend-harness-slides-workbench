...
import TeaChaRoutes, {
  ...
  teaChaRoutesTopic,
} from "./09-tea-cha-routes";
import componentSource from "./09-tea-cha-routes.tsx?raw";
import styleSource from "./09-tea-cha-routes.module.css?inline";
...
describe("Tea / Cha Routes topic protocol", () => {
  it("exports the planned facts packet, navigation, and exact transition score", () => {
    expect(teaChaRoutesTopic.id).toBe("tea-cha-routes");
    expect(teaChaRoutesTopic.topic).toEqual({
      en: "Tea / Cha",
      zh: "茶与 Cha",
    });
    expect(teaChaRoutesTopic.navigation).toEqual({
      geometry: "path",
      carrier: "tea-cha-route-lines",
      invocation: "persistent",
      feedback: "next-state-preview",
    });
    expect(teaChaRoutesTopic.sources).toBe(TEA_CHA_ROUTES_SOURCES);
    expect(teaChaRoutesTopic.transitionScore).toBe(
      TEA_CHA_ROUTES_TRANSITION_SCORE,
    );
  ...
  it("keeps five bilingual scenes aligned to the 1-3-2-4-1 curve", () => {
    ...
    expect(english.id).toBe("subway-map-of-intent");
