...
} from "./arcade-boss-fight-egg-mimicry";
import styleClasses from "./arcade-boss-fight-egg-mimicry.module.css";
import componentSource from "./arcade-boss-fight-egg-mimicry.tsx?raw";
...
const cssSource = readFileSync(
  "src/styles/arcade-boss-fight-egg-mimicry.module.css",
  "utf8",
);
...
describe("Arcade Boss Fight: egg mimicry — topic contract", () => {
  it("exports the locked topic, nest-map navigation, and authored score", () => {
    expect(eggMimicryTopic.id).toBe("egg-mimicry");
    expect(eggMimicryTopic.topic).toEqual({
      en: "Egg Mimicry",
      zh: "卵拟态",
    });
    ...
    expect(eggMimicryTopic.sources).toBe(EGG_MIMICRY_SOURCES);
    expect(eggMimicryTopic.transitionScore).toBe(EGG_MIMICRY_TRANSITION_SCORE);
  ...
  it("keeps EN and ZH metadata aligned to the 4/1/2/2/1 curve", () => {
    ...
    expect(en.id).toBe("arcade-boss-fight");
