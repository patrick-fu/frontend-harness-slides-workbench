...
import DistrictHeat, {
  ...
  districtHeatTopic,
  getMetadata,
} from "./signal-pipeline-flow-district-heat";
import componentSource from "./signal-pipeline-flow-district-heat.tsx?raw";
import styleSource from "./signal-pipeline-flow-district-heat.module.css?inline";
...
describe("Second Heat topic contract", () => {
  it("declares the planned facts packet, navigation, score, and bilingual beat curve", () => {
    expect(districtHeatTopic.id).toBe("district-heat");
    expect(districtHeatTopic.topic).toEqual({
      en: "Second Heat",
      zh: "城市余热",
    });
    expect(districtHeatTopic.navigation).toEqual({
      geometry: "path",
      carrier: "heat-pipe-loop",
      invocation: "auto-hide",
      feedback: "mechanical-displacement",
    });
    expect(districtHeatTopic.sources).toBe(DISTRICT_HEAT_SOURCES);
    expect(districtHeatTopic.transitionScore).toBe(
      DISTRICT_HEAT_TRANSITION_SCORE,
    );
    ...
    expect(english.id).toBe("signal-pipeline-flow");
