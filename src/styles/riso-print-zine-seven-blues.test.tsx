...
import RisoPrintZineSevenBlues, {
  SEVEN_BLUES_CLAIMS,
  ...
  getMetadata,
  sevenBluesTopic,
} from "./riso-print-zine-seven-blues";
import componentSource from "./riso-print-zine-seven-blues.tsx?raw";

const cssSource = readFileSync(
  "src/styles/riso-print-zine-seven-blues.module.css",
  "utf8",
) as string;
...
describe("Riso Print Zine / Seven Blues — coordinated topic contract", () => {
  it("exports the planned topic, swatch navigation, exact score, and required model", () => {
    expect(sevenBluesTopic.id).toBe("seven-blues");
    expect(sevenBluesTopic.topic).toEqual({
      en: "Seven Blues",
      zh: "七种蓝",
    });
    ...
    expect(sevenBluesTopic.sources).toBe(SEVEN_BLUES_SOURCES);
    expect(sevenBluesTopic.transitionScore).toBe(
      SEVEN_BLUES_TRANSITION_SCORE,
    );
  ...
  it("keeps English and Chinese metadata structurally aligned to 4-3-2-1-1", () => {
    ...
    for (const metadata of [en, zh]) {
      expect(metadata.id).toBe("riso-print-zine");
      expect(metadata.band).toBe("editorial-print");
