...
import issueBrief from "./issue-brief";
import type { TopicStageProps } from "../domain/topic";
import { runTopicContract } from "../testing/topic-contract";

runTopicContract(issueBrief);

const TopicStage = issueBrief.Stage;
...
describe("Issue brief — render coverage", () => {
...
describe("Issue brief — thumbnail mode", () => {
...
describe("Issue brief — navigation presence", () => {
...
describe("Issue brief — navigation behavior", () => {
  it("clicking nav calls onNavigate with (sceneId, 0)", () => {
    const { onNavigate } = renderStage({ scene: 1, beat: 0 });
    const btn = screen
      .getAllByRole("button")
      .find((el) => el.getAttribute("aria-label") === "Jump to scene 4");
...
describe("Issue brief — overflow check", () => {
...
describe("Issue brief — metadata structure", () => {
  it("returns complete metadata", () => {
    const meta = issueBrief.metadata.en;
    expect(issueBrief.styleId).toBe("maintainer-issue-brief");
    expect(issueBrief.title.en.length).toBeGreaterThan(0);
