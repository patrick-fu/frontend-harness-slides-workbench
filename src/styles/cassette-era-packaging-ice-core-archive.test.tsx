...
} from "./cassette-era-packaging-ice-core-archive";
import componentSource from "./cassette-era-packaging-ice-core-archive.tsx?raw";
...
describe("Cassette-Era Packaging / Ice-Core Archive — topic protocol and evidence", () => {
  it("exports the assigned topic, model, navigation profile, and exact transition score", () => {
    expect(iceCoreArchiveTopic.id).toBe("ice-core-archive");
    expect(iceCoreArchiveTopic.topic).toEqual({
      en: "Ice-Core Archive",
      zh: "冰芯档案",
    });
    ...
    expect(iceCoreArchiveTopic.sources).toBe(ICE_CORE_ARCHIVE_SOURCES);
    expect(iceCoreArchiveTopic.transitionScore).toBe(
      ICE_CORE_ARCHIVE_TRANSITION_SCORE,
    );
  ...
  it("keeps both languages aligned to the planned 4-3-2-1-1 beat curve", () => {
    for (const language of ["en", "zh"] as const) {
      ...
      expect(metadata.id).toBe("cassette-era-packaging");
