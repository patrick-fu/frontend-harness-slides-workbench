...
} from "./lichen-partners";
import componentSource from "./lichen-partners.tsx?raw";
import cssSource from "./lichen-partners.module.css?inline";
...
describe("Lichen Partners topic contract", () => {
  it("exports the assigned ID, model, navigation profile, and transition score", () => {
    expect(topic.id).toBe("lichen-partners");
    ...
    expect(topic.transitionScore).toBe(LICHEN_TRANSITION_SCORE);
  ...
  it("keeps source IDs and claim IDs reciprocal, authoritative, and parseable", () => {
    ...
    if (topic.evidence.kind !== "facts") {
      throw new Error("Lichen Partners must retain factual Evidence.");
    }
    expect(topic.evidence.sources).toBe(LICHEN_SOURCES);
    expect(LICHEN_SOURCES.length).toBeGreaterThanOrEqual(3);

    for (const source of LICHEN_SOURCES) {
      const topicSource: Source = source;
