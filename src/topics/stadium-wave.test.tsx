...
import componentSource from "./stadium-wave.tsx?raw";

runTopicContract(stadiumWave);

const TopicStage = stadiumWave.Stage;
...
describe("Stadium Wave topic protocol", () => {
  it("exports the planned semantic topic and navigation profile", () => {
    expect(stadiumWave.id).toBe("stadium-wave");
    expect(stadiumWave.title).toEqual({
      en: "Stadium Wave",
      zh: "看台人浪",
    });
    expect(stadiumWave.navigation).toEqual({
      geometry: "spatial-node",
      carrier: "stadium-seating-array",
      invocation: "auto-hide",
      feedback: "active-glow",
    });
    expect(stadiumWave.evidence).toMatchObject({
      kind: "facts",
      sources: STADIUM_WAVE_SOURCES,
    });
    expect(stadiumWave.transitionScore).toBe(
      STADIUM_WAVE_TRANSITION_SCORE,
    );
  ...
  it("keeps five localized scenes structurally aligned", () => {
    const english = stadiumWave.metadata.en;
    const chinese = stadiumWave.metadata.zh;

    expect(stadiumWave.styleId).toBe("sketch-board-emoji");
