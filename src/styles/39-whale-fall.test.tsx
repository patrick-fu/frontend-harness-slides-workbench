...
describe("Style 39 / Whale Fall — topic contract", () => {
  it("exports the planned topic, facts packet, navigation, and exact score", () => {
    expect(whaleFallTopic.id).toBe("whale-fall");
    expect(whaleFallTopic.topic).toEqual({
      en: "Whale Fall",
      zh: "鲸落",
    });
    expect(whaleFallTopic.navigation).toEqual({
      geometry: "card-miniature",
      carrier: "whale-fall-filmstrip",
      invocation: "keyboard-focus",
      feedback: "next-state-preview",
    });
    expect(whaleFallTopic.sources).toBe(WHALE_FALL_SOURCES);
    expect(whaleFallTopic.transitionScore).toBe(
      WHALE_FALL_TRANSITION_SCORE,
    );
    expect(WHALE_FALL_TRANSITION_SCORE).toEqual({
      "1->2": "dip-to-color",
      "2->3": "dolly-pull",
      "3->4": "focus-swap",
      "4->5": "crossfade",
  ...
  it("ships at least three claim-scoped authoritative sources with boundaries", () => {
    expect(WHALE_FALL_SOURCES.length).toBeGreaterThanOrEqual(3);
    for (const source of WHALE_FALL_SOURCES) {
  ...
  it("keeps EN/ZH metadata aligned to five scenes and the 1/1/2/3/4 curve", () => {
    ...
    expect(en.id).toBe("widescreen-title-card");
...
describe("Style 39 / Whale Fall — render and beat states", () => {
...
describe("Style 39 / Whale Fall — transitions and filmstrip", () => {
  it("renders all four authored transition edges", async () => {
    ...
    for (let scene = 2; scene <= 5; scene += 1) {
      view.rerender(
        <div style={{ width: 1920, height: 1080, containerType: "size" }}>
          <WhaleFall {...baseProps} scene={scene} />
        </div>,
      );
      await waitFor(() => {
        expect(
...
describe("Style 39 / Whale Fall — stage safety", () => {
