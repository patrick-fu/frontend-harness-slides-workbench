...
import componentSource from "./freedive.tsx?raw";
import cssSource from "./freedive.module.css?inline";

runTopicContract(freedive);

const TopicStage = freedive.Stage;
...
function renderNativeTouchStage(overrides: Partial<TopicStageProps> = {}) {
  const onNext = vi.fn();
  const onPrev = vi.fn();
...
describe("Spotlight Quote Poster / Freedive — coordinated topic contract", () => {
  it("exports the planned topic, ambient footlight navigation, research packet, and score", () => {
    ...
    expect(freedive.modelId).toBe("GPT 5.6 Sol");
    expect(freedive.navigation).toEqual({
      geometry: "ambient",
      carrier: "footlight-notches",
      invocation: "auto-hide",
      feedback: "history-trail",
    });
    expect(freedive.evidence).toMatchObject({
      kind: "mixed",
      sources: FREEDIVE_SOURCES,
    });
    expect(freedive.transitionScore).toBe(FREEDIVE_TRANSITION_SCORE);
  ...
  it("keeps the planned EN/ZH scene contract, including Scene 5 beat 0", () => {
    const english = freedive.metadata.en;
    const chinese = freedive.metadata.zh;

    expect(freedive.styleId).toBe("spotlight-quote-poster");
