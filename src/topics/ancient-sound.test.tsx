...
import componentSource from "./ancient-sound.tsx?raw";
import { runTopicContract } from "../testing/topic-contract";

runTopicContract(ancientSound);

const TopicStage = ancientSound.Stage;
...
const styleSource = readFileSync(
  "src/topics/ancient-sound.module.css",
  "utf8",
);
...
describe("Field Notes Report / Ancient Sound — archaeological evidence", () => {
  ...
  it("keeps the five-scene 2/4/1/3/1 metadata contract aligned in both languages", () => {
    const en = ancientSound.metadata.en;
    const zh = ancientSound.metadata.zh;

    expect(ancientSound.styleId).toBe("field-notes-report");
...
describe("Field Notes Report / Ancient Sound — source and transition contracts", () => {
  it("keeps the exact transition score after every edge settles in one continuous sequence", () => {
    ...
    expect(ancientSound.transitionScore).toBe(ancientSoundTransitionScore);
  ...
  it("carries authoritative HTTPS sources and a closed claim-to-source graph", () => {
    ...
    expect(ancientSound.evidence.kind).toBe("mixed");
    if (ancientSound.evidence.kind === "mixed") {
      expect(ancientSound.evidence.sources).toBe(ancientSoundSources);
    }
