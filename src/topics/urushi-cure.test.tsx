...
function factualSources() {
  if (topic.evidence.kind !== "facts") {
    throw new Error("Urushi Cure must retain factual Evidence.");
  }
...
describe("Urushi Cure render and localization", () => {
...
describe("Urushi Cure factual protocol", () => {
  ...
  it("carries at least three claim-scoped HTTPS sources", () => {
    ...
    for (const source of factualSources()) {
      ...
      expect(
        Boolean(source.authority || source.citation || source.title),
      ).toBe(true);
    }
  });
});

describe("Urushi Cure navigation and motion safety", () => {
