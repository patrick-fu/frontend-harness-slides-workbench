...
import cocoaFermentation, {
  COCOA_FERMENTATION_SOURCES,
  COCOA_FERMENTATION_TRANSITION_SCORE,
} from "./cocoa-fermentation";
import { runTopicContract } from "../testing/topic-contract";
import componentSource from "./cocoa-fermentation.tsx?raw";

runTopicContract(cocoaFermentation);

const TopicStage = cocoaFermentation.Stage;
const getMetadata = (language: "en" | "zh") => cocoaFermentation.metadata[language];
...
describe("Kitchen Prep Station / Cocoa Fermentation", () => {
  it("publishes the assigned topic identity", () => {
    expect(cocoaFermentation.id).toBe("cocoa-fermentation");
    expect(cocoaFermentation.title).toEqual({
  ...
  it("declares the exact navigation, transition, source, and bilingual metadata contracts", () => {
    ...
    expect(cocoaFermentation.evidence).toEqual({
      kind: "facts",
    ...
    expect(cocoaFermentation.styleId).toBe("kitchen-prep-station");
  ...
  it("renders the exact four-edge score through SpatialSceneTrack", async () => {
    ...
    for (let scene = 2; scene <= 5; scene += 1) {
      view.rerender(
        <div style={{ width: 1920, height: 1080, containerType: "size" }}>
          <TopicStage {...baseProps} scene={scene} />
        </div>,
      );
      await waitFor(() => {
        expect(
