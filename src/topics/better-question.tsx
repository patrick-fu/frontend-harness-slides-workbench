...
function useFonts(): void {
  useEffect(() => {
    const id = "better-question-fonts";
...
function SpeakerMarker({
  ...
  return (
    <div
      className={cx("better-question-speaker", `better-question-speaker-${side}`)}
      data-active={active ? "true" : "false"}
      data-beat-layout-item="true"
    >
      <span className="better-question-speaker-light" />
      <span className="better-question-speaker-label">{text(label, language)}</span>
      <span className="better-question-speaker-caption">{text(caption, language)}</span>
    </div>
  );
}
...
function DialogueTurns({
  ...
  return (
    <div className="better-question-turn-stack" data-beat-layout-item="true">
      {visibleTurns.map((turn, index) => (
        <div
          key={turn.id}
          className={cx("better-question-turn", `better-question-turn-${turn.side}`)}
          data-side={turn.side}
          data-beat-layout-item="true"
          style={{ "--turn-index": String(index) } as CSSProperties}
        >
          <div className="better-question-turn-label">{text(turn.label, language)}</div>
          <div className="better-question-turn-line">{text(turn.line, language)}</div>
          <div className="better-question-turn-note">{text(turn.note, language)}</div>
        </div>
      ))}
    </div>
  );
...
function SceneArtifact({
  ...
  if (scene.layout === "aligned" && scene.answer) {
    return (
      <div className="better-question-artifact better-question-answer" data-beat-layout-item="true">
        <div className="better-question-answer-label">{text(scene.answer.label, language)}</div>
        {beat >= 1 ? (
          <p className="better-question-answer-body">{text(scene.answer.body, language)}</p>
        ) : null}
        {beat >= 2 ? (
          <div className="better-question-chip-row">
            {scene.answer.chips.map((chip) => (
              <span key={chip.en} className="better-question-chip" data-beat-layout-item="true">
                {text(chip, language)}
              </span>
            ))}
          </div>
  ...
  return (
    <div
      className={cx("better-question-artifact", `better-question-artifact-${scene.layout}`)}
      data-beat-layout-item="true"
    >
      {scene.artifacts.slice(0, beat + 1).map((item, index) => (
        <span
          key={item.en}
          className="better-question-artifact-token"
          data-beat-layout-item="true"
...
function ScenePanel({
  ...
  return (
    <article
      ref={flip.ref}
      className="better-question-scene"
    ...
    >
      <header className="better-question-scene-header" data-beat-layout-item="true">
        <p className="better-question-kicker">{text(scene.kicker, language)}</p>
        <h1>{text(scene.title, language)}</h1>
        <p className="better-question-subtitle">{text(scene.subtitle, language)}</p>
      </header>

      <div className="better-question-dialogue-stage" data-beat-layout-item="true">
        <SpeakerMarker
          side="warm"
...
function SpotlightCueRail({
  ...
  return (
    <nav
      className="better-question-cue-rail"
      aria-label="Scene cues"
      ...
      {SCENE_IDS.map((sceneId) => {
        ...
        return (
          <button
            key={sceneId}
            type="button"
            className="better-question-cue-button"
            data-active={active ? "true" : "false"}
            onClick={() => onNavigate?.(sceneId, 0)}
            aria-label={text(target.title, language)}
            style={{ "--cue-accent": target.accent } as CSSProperties}
          >
            <span className="better-question-cue-lamp" />
            <span className="better-question-cue-label">{String(sceneId).padStart(2, "0")}</span>
          </button>
        );
      })}
    </nav>
...
function TopicStage({
  ...
  return (
    <section
      className="better-question-root"
      data-motion={motionOff ? "off" : "on"}
      data-thumbnail={isThumbnail ? "true" : "false"}
    >
      <style>{BETTER_QUESTION_STYLES}</style>
      <div className="better-question-proscenium" aria-hidden="true" />
      <div className="better-question-light better-question-light-warm" aria-hidden="true" />
      <div className="better-question-light better-question-light-cool" aria-hidden="true" />
      <div className="better-question-top-chrome" aria-hidden="true">
        <span>04 / topic</span>
      ...
      <SpatialSceneTrack
        ...
        className="better-question-track"
...
.better-question-root {
  --better-question-bg: #061019;
  --better-question-ink: #f5fbff;
  --better-question-muted: #8ca4b8;
  --better-question-panel: rgba(11, 27, 41, 0.72);
  --better-question-panel-strong: rgba(15, 39, 58, 0.9);
  --better-question-warm: #ffb15f;
  --better-question-cool: #65dcff;
  --better-question-good: #8cf2c2;
  position: relative;
  width: 100%;
  height: 100%;
  container-type: size;
  overflow: hidden;
  color: var(--better-question-ink);
...
.better-question-track {
  position: absolute;
  inset: 8.4cqh 5.4cqw 15cqh;
  z-index: 4;
}

.better-question-scene {
  position: relative;
  width: 100%;
  height: 100%;
  display: grid;
...
.better-question-scene-header {
  display: grid;
  gap: 1.2cqh;
  max-width: 70cqw;
}

.better-question-kicker {
  margin: 0;
...
.better-question-dialogue-stage {
  display: grid;
...
.better-question-scene[data-layout="loop"] .better-question-dialogue-stage,
.better-question-scene[data-layout="aligned"] .better-question-dialogue-stage {
  grid-template-columns: 12.5cqw minmax(0, 1fr) 12.5cqw;
}

.better-question-scene[data-layout="curtain"] .better-question-dialogue-stage {
  min-height: 36cqh;
}

.better-question-speaker {
  position: relative;
  display: grid;
...
.better-question-speaker[data-active="true"] {
  color: var(--better-question-ink);
...
.better-question-speaker-warm .better-question-speaker-light {
  color: var(--better-question-warm);
}

.better-question-speaker-cool .better-question-speaker-light {
  color: var(--better-question-cool);
}

.better-question-speaker-label {
  font-size: 1.1cqw;
  font-weight: 700;
  line-height: 1;
  letter-spacing: 0;
...
.better-question-speaker-caption {
...
.better-question-turn {
  ...
  background: var(--better-question-panel);
  box-shadow: 0 1.4cqh 2.8cqw rgba(0, 0, 0, 0.22);
  animation: better-question-turn-enter 520ms cubic-bezier(0.16, 1, 0.3, 1) both;
  animation-delay: calc(var(--turn-index) * 0.055s);
}

.better-question-turn::before {
  content: "";
  position: absolute;
  top: 1.5cqh;
  bottom: 1.5cqh;
...
.better-question-turn-warm::before {
  left: 1cqw;
  background: var(--better-question-warm);
}

.better-question-turn-cool::before {
  right: 1cqw;
  background: var(--better-question-cool);
}

.better-question-turn-both::before {
  left: 1cqw;
  right: 1cqw;
  width: auto;
  top: auto;
  height: 0.22cqh;
  background: linear-gradient(90deg, var(--better-question-warm), var(--better-question-cool));
}

.better-question-turn-label {
  color: var(--scene-accent);
  font-size: 0.92cqw;
  font-weight: 700;
  line-height: 1;
...
.better-question-artifact-token,
.better-question-chip {
  display: inline-flex;
  align-items: center;
  justify-content: center;
  min-height: 4.2cqh;
  ...
  line-height: 1.1;
  letter-spacing: 0;
  animation: better-question-token-enter 520ms cubic-bezier(0.16, 1, 0.3, 1) both;
  animation-delay: calc(var(--token-index) * 0.07s);
}

.better-question-artifact-friction .better-question-artifact-token {
  border-color: rgba(255, 111, 95, 0.62);
  background: rgba(77, 21, 24, 0.64);
}

.better-question-artifact-loop {
  display: grid;
  grid-template-columns: repeat(2, minmax(0, 1fr));
  align-items: stretch;
}

.better-question-artifact-loop .better-question-artifact-token {
  border-radius: 0.85cqw;
  min-height: 6.4cqh;
}

.better-question-answer {
  display: grid;
  gap: 1.3cqh;
  justify-items: center;
...
.better-question-answer-label {
  color: var(--better-question-good);
  font-size: 0.98cqw;
  font-weight: 700;
  line-height: 1;
  text-transform: uppercase;
...
.better-question-cue-rail::before {
  content: "";
  position: absolute;
  left: 3cqw;
  right: 3cqw;
...
.better-question-cue-button:focus-visible {
  outline: 0.16cqw solid var(--cue-accent);
  outline-offset: 0.45cqw;
  border-radius: 1cqw;
}

.better-question-cue-lamp {
  width: 1.35cqw;
  height: 1.35cqw;
  border-radius: 50%;
...
.better-question-cue-button[data-active="true"] {
  color: var(--cue-accent);
}

.better-question-cue-button[data-active="true"] .better-question-cue-lamp {
...
.better-question-root[data-thumbnail="true"] .better-question-track {
  inset: 7.4cqh 5cqw 7.4cqh;
}

.better-question-root[data-thumbnail="true"] .better-question-top-chrome {
  display: none;
}

.better-question-root[data-thumbnail="true"] .better-question-scene {
  padding: 4cqh 4cqw;
  gap: 1.6cqh;
}

.better-question-root[data-thumbnail="true"] .better-question-scene h1 {
  font-size: 4cqw;
}

.better-question-root[data-thumbnail="true"] .better-question-subtitle,
.better-question-root[data-thumbnail="true"] .better-question-turn-note,
.better-question-root[data-thumbnail="true"] .better-question-speaker-caption {
  display: none;
}

.better-question-root[data-motion="off"] *,
.better-question-root[data-motion="off"] *::before,
.better-question-root[data-motion="off"] *::after {
  animation-duration: 0s !important;
  transition-duration: 0s !important;
}

@keyframes better-question-turn-enter {
  from {
    opacity: 0;
...
@keyframes better-question-token-enter {
  from {
    opacity: 0;
